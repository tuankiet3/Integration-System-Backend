    ...
    public class TimekeepingController : Controller
    {
        ...
        public async Task<IActionResult> GetAllAttendances()
        {
            try
            {
                ...
                _logger.LogInformation("Trả về {Count} bản ghi chấm công.", ((List<AttendanceModel>)attendanceRecords).Count);
        ...
        public async Task<IActionResult> UpdateAttendance(int attendanceId, [FromBody] UpdateAttendanceDto attendanceDto)
        {
            ...
            if (existingRecord == null)
            {
                ...
                return NotFound(new { message = $"Không tìm thấy bản ghi chấm công với ID {attendanceId}." });
            }
            ...
            {
                _logger.LogError(ex, "Lỗi không mong muốn trong action UpdateAttendance cho ID {AttendanceId}: {@AttendanceDto}", attendanceId, attendanceDto);
                return StatusCode(StatusCodes.Status500InternalServerError, "Đã có lỗi xảy ra phía máy chủ khi cập nhật chấm công.");
            }
        }
