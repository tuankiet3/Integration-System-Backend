    ...
    public class AttendanceDAL
    {
        ...
        public async Task<IEnumerable<AttendanceModel>> GetAttendancesAsync()
        {
            ...
            try
            {
                ...
                _logger.LogInformation("Lấy thành công {Count} bản ghi chấm công.", attendanceList.Count);
            }
            catch (Exception ex)
            {
        ...
        public async Task<AttendanceModel?> GetAttendanceByAttendanceIdAsync(int attendanceId)
        {
            ...
            try
            {
                ...
                if (attendance != null)
                {
                    _logger.LogInformation("Lấy thông tin chấm công ID: {AttendanceId} thành công.", attendanceId);
                }
                else
                {
                    _logger.LogWarning("Không tìm thấy bản ghi chấm công với ID: {AttendanceId}", attendanceId);
                }
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Lỗi khi lấy thông tin chấm công ID: {AttendanceId}.", attendanceId);
                throw;
            }
            finally
            {
        ...
        public async Task<bool> UpdateAttendanceAsync(int attendanceId, UpdateAttendanceDto attendanceDto)
        {
            ...
            try
            {
                ...
                if (rowsAffected > 0)
                {
                    _logger.LogInformation("Cập nhật thành công bản ghi chấm công ID: {AttendanceId}.", attendanceId);
                    return true;
                }
                else
                {
                    _logger.LogWarning("Không tìm thấy bản ghi chấm công ID: {AttendanceId} để cập nhật.", attendanceId);
                    return false;
                }
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Lỗi khi cập nhật bản ghi chấm công ID: {AttendanceId}.", attendanceId);
                return false;
            }
        }
        private AttendanceModel MapReaderToAttendanceModel(MySqlDataReader reader)
        {
            ...
            {
                _logger.LogError(ex, "Lỗi khi đọc hoặc chuyển đổi AttendanceMonth.");
            }
